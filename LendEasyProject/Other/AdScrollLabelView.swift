...
class AdScrollLabelView: UIView {
    ...
    lazy final var adImageView:UIImageView = {
        let _adImageView:UIImageView = UIImageView.init(frame: CGRect.init(x: 12, y: 0, width: 0, height: 0))
    ...
    func beginScroll(textArray:Array<String>) {
        ...
        if timer.isValid {
            if textArray.count <= 1{
                self.firstLabel.setAttributedTextWithHtmlString(textArray[0])
                self.firstLabel.tag = 10000
                if(self.ifNumbers == true){
...
//                self.secondLabel.text = textArray[1]
                self.secondLabel.setAttributedTextWithHtmlString(textArray[1])
                if(self.ifNumbers == true){
                    self.firstLabel.numberOfLines = 0
                    self.secondLabel.numberOfLines = 0
                }
...
//                self.secondLabel.text = self.adTextArray[self.textIndex]
                self.secondLabel.setAttributedTextWithHtmlString(self.adTextArray[self.textIndex])
                self.secondLabel.tag = self.textIndex + 10000
            }else{
                self.firstLabel.frame.origin.y = -firstLabelY
//                self.firstLabel.text = self.adTextArray[self.textIndex]
                self.firstLabel.setAttributedTextWithHtmlString(self.adTextArray[self.textIndex])
